...
import { 
  faSuitcaseRolling, faClipboardList, faTicket, faWallet, faGear, faBook 
...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'journal': return <JournalPage />; // ✅ 2. 補回：切換邏輯
  ...
  return (
    <div className="app-container font-sans bg-[#FBFBFB] min-h-screen text-[#5E5340] selection:bg-orange-200">
      <div className="max-w-md mx-auto min-h-screen relative shadow-2xl bg-[#FFFAFA]">
        ...
        {/* 底部導覽列 */}
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-[400px] bg-white/90 backdrop-blur-md border-2 border-[#F2F4E7] rounded-3xl shadow-xl flex justify-between items-center px-2 py-3 z-50 overflow-x-auto">
          {[
            ...
            { id: 'journal', icon: faBook, label: '日誌' }, // ✅ 3. 補回：按鈕 (放在設定前面)
